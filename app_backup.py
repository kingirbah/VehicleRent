...
def index():
    ...
    for vehicle in vehicles_raw:
        ...
        if start_date and end_date:
            ...
            # No dates selected, show as available
            vehicle_dict['available'] = True
...
def admin_catalog():
    ...
    if category == 'all':
        vehicles = db.execute('SELECT * FROM vehicles ORDER BY category, created_at DESC').fetchall()
    else:
        vehicles = db.execute('SELECT * FROM vehicles WHERE category = ? ORDER BY created_at DESC', (category,)).fetchall()
    ...
    if request.method == 'POST':
        try:
            ...
            db.execute('''INSERT INTO vehicles 
                (name, type, cc, license_plate, category, price_day, price_3day, price_weekly, price_monthly, image_url) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
                (request.form['name'],
                 request.form['type'],
                 request.form['cc'],
...
def admin_detail(id):
    ...
    now = datetime.now()
    calendar_data = get_calendar_data(id, now.year, now.month)
...
def admin_on_rent():
    ...
    on_rent = db.execute(query, (today, today)).fetchall()
...
def admin_add_booking(vehicle_id):
    ...
    try:
        ...
        db.execute('''INSERT INTO bookings 
            (booking_number, vehicle_id, customer_name, ic_number, nationality, location, destination,
             start_date, pickup_time, end_date, return_time, status) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
            (booking_number,
             vehicle_id,
...
def admin_edit_booking(id):
    ...
    try:
        db = get_db()
        booking = db.execute('SELECT vehicle_id FROM bookings WHERE id = ?', (id,)).fetchone()
        ...
        db.execute('''UPDATE bookings SET 
            customer_name=?, ic_number=?, nationality=?, location=?, destination=?,
            start_date=?, pickup_time=?, end_date=?, return_time=?, status=?
            WHERE id=?''', 
            (request.form['customer_name'],
             request.form.get('ic_number', ''),
...
def admin_all_bookings():
    ...
    now = datetime.now()
    month_filter = request.args.get('month', now.strftime('%Y-%m'))
    ...
    if vehicle_filter and vehicle_filter != 'all':
        try:
            ...
            pass  # Skip invalid vehicle filter
